...
export default function ForgotPassword() {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      setSuccess(response.message || 'Email sent successfully. Please check your inbox.');
    } catch (err) {
      ...
      <Card className="w-full max-w-md shadow-sm border-zinc-200 dark:border-zinc-800 rounded-3xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {success && (
              <div className="p-3 text-sm text-green-600 bg-green-50 dark:bg-green-950/50 rounded-xl border border-green-200 dark:border-green-900">
                {success}
              </div>
            )}
